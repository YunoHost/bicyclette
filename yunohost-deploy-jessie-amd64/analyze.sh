...
MATCH_PATTERN="(fail|warning|no such|not found|error|closed connection|unknown|traceback|unable)"
...
N30=`grep -v -E "$IGNORE_PATTERN" ./3*.{log,err} | grep -o -i -E "$MATCH_PATTERN" | wc -l`
...
grep -v -E "$IGNORE_PATTERN" ./3*.{log,err} | grep -C2 -i -E "$MATCH_PATTERN" | sed -r "s/\x1B\[([0-9]{1,2}(;[0-9]{1,2})?)?[m|K]//g" > $LOG_DIR/30_basictests.log
grep -v -E "$IGNORE_PATTERN" ./*.{log,err}  | grep -C2 -i -E "$MATCH_PATTERN" | sed -r "s/\x1B\[([0-9]{1,2}(;[0-9]{1,2})?)?[m|K]//g" > $LOG_DIR/report.log
...
cat << EOF > $LOG_DIR/summary.log
{
    ...
    "basictests"  : { "errors" : $N30, "result": "`cat 399_result`" }
}
EOF
